...
class ReadMerger extends Transform {
  ...
  fuse(...src) {
    ...
    function pauseStream(xtream) {
      if (!xtream._readableState && xtream.pipe) xtream = xtream.pipe(new PassThrough(options));
      if (!xtream._readableState || !xtream.pause || !xtream.pipe) {
        throw new Error('Only readable stream can be merged.');
      }
      xtream.pause();
      return xtream;
    }
    src.forEach(item => reader.push(Array.isArray(item) ? [pauseStream(item[0]), item[1] || {}] : [item, {}]));
    return reader;
  }
}
